...
#ifdef USAR_TAREA_LED
	/* Create the other task in exactly the same way. */
	xTaskCreate(	vTareaLED,	/* Pointer to the function that implements the task. */
					"Task 2",
					240,		/* Stack depth in words. */
					(void*) 250,/* Parametro de la tarea. */
					1,			/* This task will run at priority 1. */
					NULL );
#endif
...
void vTareaLED (void* pvParameters){
	...
	// Inicializacion
	demora = (portTickType) pvParameters;
...
//-----------------------------------------------------------------------------
	...
	for( ;; )
	{
		GPIO_SetValue	(LED_PORT, LED_BIT);
			vTaskDelay	(demora / portTICK_RATE_MS);
